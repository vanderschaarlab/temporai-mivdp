...
from .common import pd_v2_compat_append
...
def dataframe_from_csv(
    ...
    header=0,
    index_col=0,
    chunksize=None,  # pylint: disable=unused-argument
):
...
def preproc_proc(
    dataset_path: str, cohort_path: str, time_col: str, dtypes: Optional[dict], usecols: list
) -> pd.DataFrame:
...
def preproc_out(
    dataset_path: str, cohort_path: str, time_col: str, dtypes: Optional[dict], usecols: Optional[list]
) -> pd.DataFrame:
    """Function for getting hosp observations pertaining to a pickled cohort.
    Function is structured to save memory when reading and transforming data.
    """
...
def preproc_chart(
    ...
    for chunk in tqdm(
        ...
        if df_cohort.empty:
            ...
            df_cohort = pd_v2_compat_append(df_cohort, chunk_merged, ignore_index=True)
...
def preproc_icd_module(
    ...
    def get_module_cohort(
        module_path: str,
        cohort_path: str,  # pylint: disable=unused-argument
    ):
    ...
    def _standardize_icd(mapping, df, root=False):
        """Takes an ICD9 -> ICD10 mapping table and a diagnosis dataframe; adds column with converted ICD10 column"""
    ...
    if icd_map_path:
        ...
        # print(icd_map)
        _standardize_icd(icd_map, module, root=True)
        print(
            "# Unique ICD-9 codes",
...
def pivot_cohort(
    ...
    if use_mlb:
        ...
        pivot_df = pd.pivot_table(  # type: ignore
            pivot_df,
            index=["subject_id", "label", "timedelta"],
            values=pivot_df.columns[3:],  # type: ignore
            aggfunc=np.max,
        )
    else:
        if max_features:
            top_features = pd.Series(
                pivot_df[["subject_id", target_col]].drop_duplicates()[target_col].value_counts().index[:max_features],
                name=target_col,
            )
