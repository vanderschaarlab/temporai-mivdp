...
def get_diagnosis_icd(module_path: str) -> pd.DataFrame:
    ...
    return pd.read_csv(module_path + "/hosp/diagnoses_icd.csv.gz", compression="gzip", header=0)


# TODO: Investigate - should this not return `diag` df?
def standardize_icd(mapping: pd.DataFrame, diag: pd.DataFrame, map_code_col="diagnosis_code", root=True) -> None:
...
def preproc_icd_module(module_path: str, ICD10_code: str, icd_map_path: str) -> pd.DataFrame:
